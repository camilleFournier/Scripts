...
def doExperiment(isFirst, nbMeasures, interval):
    # Runs the component
    file = open('galaxyS6_70_real_'+str(interval)+'.txt', 'at')
    if (isFirst):
        file.write('Clock, Total frames rendered, Janky frames, CPU%, PSS \n')
    
    # Number of experiments
    for loop in range(nbMeasures):
        clock = []
        gfxinfo = []
        cpuinfo = []
        meminfo = []
        ...
        for loop in range(10):
            MonkeyRunner.sleep(interval) #wait little more than 5min so cpuinfo is updated
    ...
    file.close()
# isFirst experience, nbExperiments, time interval between measures
doExperiment(False, 3, 910)
